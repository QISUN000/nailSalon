            ...
            <tr key={rowIndex} className="border-b border-gray-200 hover:bg-gray-100">
              {displayColumns.map((column) => (
                <td key={column.key} className="py-3 px-6 text-left whitespace-nowrap">
                  {column.key === 'status' ? (
                    <span className={`flex items-center ${row[column.key] === 'APPROVED' ? 'text-green-600' : 'text-yellow-600'}`}>
                      <span className={`h-2.5 w-2.5 rounded-full mr-2 ${row[column.key] === 'APPROVED' ? 'bg-green-600' : 'bg-yellow-600'}`}></span>
                      {row[column.key]}
                    </span>
                  ) : column.key === 'action' ? (
